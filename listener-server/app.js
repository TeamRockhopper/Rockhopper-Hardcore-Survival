...
app.post('/', verifyPostData, async function (req, res) {
	try {
		...
		const out = fs.openSync(logfile, 'a');
		const err = fs.openSync(logfile, 'a');
		spawn('java', [ '-Dfml.queryResult=confirm', '-server', '-XX:+UseG1GC', '-XX:MaxGCPauseMillis=100', '-XX:+UseStringDeduplication', '-XX:+UseAES', '-XX:+UseAESIntrinsics', '-Xmx24G', '-Xms8G', '-XX:UseSSE=3', '-jar', '~/mc-rockhopper-survival/modpack.jar', 'nogui' ], {
			detached: true,
			stdio: [ 'ignore', out, err ]
		}).unref();
